/** biome-ignore-all lint/complexity/noThisInStatic: Fork of @rekog-labs/MCP-Nest */
/** biome-ignore-all lint/correctness/noUnusedPrivateClassMembers: Fork of @rekog-labs/MCP-Nest */
...
export class McpModule {
   ...
   */
  private readonly __isMcpModule = true;

  public static forRoot(options: McpOptions): DynamicModule {
    ...
    mergedOptions.messagesEndpoint = normalizeEndpoint(mergedOptions.messagesEndpoint);
   ...
   */
  public static forRootAsync(options: McpModuleAsyncOptions): DynamicModule {
    const moduleId = `mcp-module-${instanceIdCounter++}`;
    ...
    return {
      ...
      controllers: [],
      providers: [...asyncProviders, ...baseProviders, ...(options.extraProviders ?? [])],
      exports: [McpRegistryService, McpSseService, McpStreamableHttpService],
    };
  }

  private static createAsyncProviders(options: McpModuleAsyncOptions): Provider[] {
    if (options.useFactory) {
      return [
        {
          provide: 'MCP_OPTIONS',
          useFactory: async (...args: unknown[]) => {
            const resolved: McpAsyncOptions = await options.useFactory!(...args);
  ...
  private static createProvidersFromOptions(options: McpOptions, moduleId: string): Provider[] {
